						...
						new Promise(function (resolve, reject) {
							Ajax.remoteCallMode('fort_battleresultpage', 'get_battle', {
								...
								for (n = 0; n < jsonAttack.length; n++) {
									...
									if (resp.stats.result.outcome == "DEFENDER_WIPED" || resp.stats.result.outcome == "FLAGLOST") {
										estimatedBonds++
									};
								...
								for (n = 0; n < jsonDefence.length; n++) {
									...
									var date = new Date(resp.stats.result_date * 1000 + 7200000);
									var estimatedBonds = 1;
									if (resp.stats.result.outcome == "FINALROUND" || resp.stats.result.outcome == "ATTACKER_WIPED") {
										estimatedBonds++
									};
